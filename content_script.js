...
$(document).ready(function () {
	...
	// エディタの幅と高さを（強制的に）変える
	// 一般＞説明、コンテンツのエリア、フォーラムの件名、フォーラムのメッセージ
	const expand_fitem_ids = ["fitem_id_introeditor", "fitem_id_page", "fitem_id_subject", "fitem_id_message"];
	$.each(expand_fitem_ids, function (idx, val) {
		$('#' + val + ' .col-md-3').removeClass("col-md-3").addClass("col-md-0");
		$('#' + val + ' .col-md-9').removeClass("col-md-9").addClass("col-md-12");
	});
	// エディタの高さをそれぞれ設定した値に引き伸ばす (査読報告や判定結果の説明、コンテンツ、および、フォーラムのメッセージ)
	const editor_ids = { 'id_introeditoreditable': 500, 'id_pageeditable': 1000, 'id_messageeditable': 600 };
	$.each(editor_ids, function (val, height) {
		$('#' + val).height(height);
	});

	// フォーラムの返信フィールド（テキストエリア）
